...
type UserActionCallback = (adapter: ApiAdapter, request: Request, userId: string, files: string[]) => Promise<any>
...
export class ExpressApiHelper {
  ...
  //   } else {
  //     response.status(ResponseCodes.UNAUTHORIZED).send(Error.create(Error.Types.apiKeyMissing))
  //     return
  //   }
